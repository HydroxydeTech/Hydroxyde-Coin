...
#if defined(NDEBUG)
#error "HDRX cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uhydroxyde) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight, uint32_t nTime)
{
    if (nHeight == 0) {
      ...
      // POS Year 1
    } else if (nHeight <= 5000000 && nHeight > Params().LAST_POW_BLOCK()) {
        return 1 * COIN;
...
{
    RenameThread("hydroxyde-scriptch");
    ...
    if (block.IsProofOfStake()) {
        ...
        for (unsigned int i = 2; i < block.vtx.size(); i++)
            ...
            if(nHeight > 1) { // exclude premine
                ...
                CScript scriptDevPubKeyIn  = CScript() << Params().xHDRXDevKey() << OP_CHECKSIG;
