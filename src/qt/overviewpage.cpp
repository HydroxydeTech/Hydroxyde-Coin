...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::HDRX)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_HDRX4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_HDRX, SIGNAL(clicked()), this, SLOT(openMyAddresses()));
       
    }

    // update the display unit, to not use the default ("HDRX")
    updateDisplayUnit();
}
...
void OverviewPage::updatBlockChainInfo()
{
    ...
    double BlockRewardHDRX =  static_cast<double>(BlockReward/COIN);
    //int64_t HDRXSupply = chainActive.Tip()->nMoneySupply / COIN;
    ...
    ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardHDRX));
    //ui->label_HDRXSupply_value->setText(QString::number(HDRXSupply));
}
