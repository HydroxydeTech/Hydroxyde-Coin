...
*Setup instructions for a gitian build of HDRX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HDRX
...
Clone the git repositories for HDRX and gitian and then checkout the HDRX version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the HDRX repository
...
To build HDRX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the HDRX repository.

This may take a long time as it also builds the dependencies needed for each descriptor.
These dependencies will be cached after a sHDRXessful build to avoid rebuilding them when possible.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/HDRX/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/Hydroxyde/HDRX
...
For example:
```bash
URL=https://github.com/Hydroxyde/HDRX.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit HDRX=${COMMIT} --url HDRX=${URL} ../HDRX/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit HDRX=${COMMIT} --url HDRX=${URL} ../HDRX/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit HDRX=${COMMIT} --url HDRX=${URL} ../HDRX/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/HDRX-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/HDRX-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/HDRX-build.assert
```
...
[HDRX/gitian.sigs](https://github.com/Hydroxyde/gitian.sigs/) repository, or if that's not possible to create a pull
request.
